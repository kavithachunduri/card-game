...
function buildDeck(){
 ...
 for (j=0;j<=suits.length-1;j++){
  ...
  let  objDeck = {suit:suits[j] , rank:ranks[i], value: i+1};
        deck.push(objDeck);
...
// 3. Using "let" declare three new variables: currentIndex, whos value should equal the length of the deck array, and two more: temporaryValue 
//and randomIndex, each of which should currently have no value assigned.
...
// 10. Review the code from steps 7,8, and 9, and leave a comment explaining what you believe those lines of code are doing as they swap 
//assignments of values between them.
...
function shuffle(deck) {
  ...
  while(currentIndex!=0){
   randomIndex = Math.floor(Math.random() * currentIndex);
    ...
    // we are shuffeling cards in the stack from one place to another place
    //for example current index is 52 which is card "K" and random index is 5 which is card with number "5"
    //now with shuffle Card "5" goes to "K" card position and vice versa...
    //this will applies to all 52 cards  that is satisfying while condition
    currentIndex -= 1;
  }
...
var newShuffleDeck = shuffle(deck);
...
// 5. Done.
function greet(){
  let name=getInput('Hi enter your name ');
  console.log(name);
  return name;
}
...
// 3. declare a variable called "input" that uses getInput() to ask the user if they think the next card will be 
//higher (h) or lower (l) than their current card and stores the user's response.
...
console.log(`card1 suit is${card1.suit } rank is ${card1.rank}`);
//console.log(`card2 suit is${card2.suit } rank is ${card2.rank}`);
let input= getInput('do you think the next card will be higher(h) or lower(l) then your current card');
let x = compare(card1.value,card2.value) ;
if (input === 'h'){
  ...
  getInput('you should guess higher or lower .you getno points for this round');
  return false;
}
}
// STEP SIX - Let's play!
// 1. declare a function called playGame
// 2. declare a variable called deck (it's okay to reuse -- remember scope!) that takes the result of the shuffle function. Remember that the 
// shuffle function needs to take the results one of our other functions as its argument...
...
// 9. Close the conditional statement and assign nextCard to currentCard. You may have to write this as the type of variable that's 
   //always global...
...
// 11. Write a line of code to execute the playGame function.
  function playGame(){
   let deckNew= shuffle(deck);

   let playerName= greet();
   let score=0;
   let currentCard = deckNew[deckNew.length-1];

  while(score<5 &&  score < deckNew.length-1){

    deckNew.splice(deckNew.length-1,1);

    let nextCard = deckNew[deckNew.length-1];

    let x = guess(currentCard,nextCard) ;
if (x > 0){
   
  score += 1;
  console.log('Congratulations and your score is'+score);
}
else if (x <= 0){
console.log('you are wrong and you got no points');
}

currentCard = nextCard;

  }
  deckNew.length ==0 ? console.log('you are out of cards and you lost'): console.log('you won ')
  }
  playGame();